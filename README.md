# 🌳 마음 정원: 솔로몬의 후계자 (Mind Garden v4.0)

> **"감정을 이해하는 마음이 모든 법의 뿌리다."**
>
> 아이들의 감정이 자라는 정원에서, 공감의 빛으로 솔로몬 왕국을 되살리고, **7대 도장**에서 배움을 닦는 정서·법·교과 통합 게임.

## 🌱 v4.0 대확장 — "자라는 정원" (원예치료 + 공감 + 숨겨진 수학)

기존 "감정→식물 1개 등장"을 **진짜 키우고 가꾸는 살아있는 정원**으로 확장했습니다.
> **핵심 철학: "아이는 정원을 가꾼다고 생각하지만, 사실은 수학을 배우고 마음을 치유받는다."**

- **🌿 식물 성장 6단계** — 씨앗→새싹→줄기→꽃봉오리→활짝→열매. 돌봄 포인트 + 시간으로 자라며, 기존 60종 아트를 단계별로 재사용.
- **💧 돌보기 5종(원예치료)** — 물주기·햇빛·잡초·대화·영양. 식물과 대화하며 감정을 표현하고, 별명을 지어 애착을 형성. 식물 탭 → 돌보기 시트.
- **🧮 숨겨진 수학** — 물방울 덧셈·씨앗 나눗셈·패턴·수확 곱셈 등을 **학년별 난이도**로 자연스럽게. 모두 **선택적 보너스**, 틀려도 따뜻한 격려뿐. 부모 리포트에 학습 통계·약점 가이드.
- **🏡 정원 레벨 6단계 + 꾸미기** — 텃밭→천상의 정원. 장식 22종 + 테마 5종을 사서 **드래그로 배치**. 레벨업 컷신.
- **🦋 살아있는 생태계** — 생물 8종(나비·꿀벌·새·무당벌레·반딧불이·개구리·토끼·달팽이) + 날씨 5종(비는 자동 물주기). **정원 일기**로 식물 성장과 마음의 변화를 시각화. 열매 수확 → 씨앗 순환.
- **🔒 어린이 보호 원칙** — **식물은 절대 죽지 않음**(시들기만, 물 주면 회복) · 수학 강요 없음 · 틀려도 혼나지 않음 · 기존 데이터 100% 보존(자동 마이그레이션).
